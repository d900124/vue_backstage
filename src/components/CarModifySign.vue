<template>
  ...
  <div class="col-5" style="padding: 0px 0px;display: flex; justify-content: flex-start;align-items: center;">
    ...
    <div style="display: flex; align-items: center;">
      ...
      <!----------------品牌查詢 --------------------------->
      <el-select v-model="findBrand" clearable placeholder="Brand" size="small"
        style="width: 130px; margin-right: 20px;">
        <el-option v-for="option in brandOptions" :key="option.value" :label="option.label" :value="option.value" />
      </el-select>

      <!-------------- 車輛查詢 ----------------------->
      <el-input v-model="input" clearable placeholder="modelName" size="small"
        style="width: 130px; margin-right: 20px;" />

      <!-------------- 模糊查詢鈕 ---------------------->
      <div class="btm-div" style="display: flex; align-items: center;" @click="performSearch">
        <font-awesome-icon icon="magnifying-glass" size="" style="color: #a33238; padding: 0; margin-right: 5px;" />
  ...
  <div class="col-10">
      ...
      <table class="table">
        ...
        <tbody class="table-group-divider">
          ...
          <tr v-for="carData in carDatas" :key="carData.id" :carData="carData">
            ...
            <el-button type="danger" style="background-color:#a33238; color:white" @click="changeId(carData.id)"
              round>修改</el-button>
          </tr>
        </tbody>
      </table>
    ...
    <div class="pagination-container">
      <el-pagination style="margin: 10px 0px;" layout="prev, pager, next" :total="total"
        :current-page.sync="currentPage" :page-size="5" @current-change="handlePageChange"></el-pagination>
    </div>
  </div>
  <div class="col-1"></div>
</template>
...
const updatePagedResults = (allResults) => {
  const startIndex = (currentPage.value - 1) * 4; // 假設每頁5項
