<template>
  <CarImage :imageByCarIdDatas=imageByCarIdDatas :mainImageUrl="mainImageUrl" :listImageUrl="listImageUrl"
    :carId="carId" style="height: 50%; overflow: hidden;"></CarImage>
  <!-- =======================圖片上傳區塊======================= -->
  ...
  <div class="d-flex flex-row">
    <div class="p-2">
      <div class="d-flex flex-column" v-for="carData in carDatas" :key="carData.id" :carData="carData">
        ...
        <input style="display: none;" id="carId1" type="text" name="carId1" v-model="carData.id"
          @input="doInput('id', $event)" required />

        <!-- =======================車型======================= -->
        <label>車型</label>
        <select name="carinfoId" v-model="carData.carinfoId" @change="doInput('carinfoId', $event)" required>
          <option v-for="carinfoData in carinfoDatas" :key="carinfoData.id" :carinfoData="carinfoData"
            :value="`${carinfoData.id}`">{{ carinfoData.modelName }}</option>
        </select>
        ...
        <!-- =======================出廠年份======================= -->
        <label>年分</label>
        <input type="text" name="productionrYear" v-model="carData.productionYear"
          @input="doInput('productionYear', $event)" required />
      </div>
    </div>
    ...
    <div class="p-2">
      <div class="d-flex flex-column" v-for="carData in carDatas" :key="carData.id" :carData="carData">
        ...
        <select name="customerId" v-model="carData.customerId" @change="doInput('customerId', $event)" required>
          <option v-for="customer in customers" :key="customer.id" :customer="customer" :value="`${customer.id}`">{{
        ...
        <input type="text" name="conditionScore" v-model="carData.conditionScore"
          @input="doInput('conditionScore', $event)" required />
        ...
        <input type="text" name="state" v-model="carData.state" @input="doInput('state', $event)"
          required /><!--注意這邊不要有可以下架的選項-->
        ...
        <label>上架日期</label>
        <input type="date" name="launchDate" v-model="carData.launchDate" @input="doInput('launchDate', $event)"
          required />
  ...
  <el-dialog v-model="pricsVisible" width="350" :show-close="false">
    ...
    <template #footer>
      <div class="dialog-footer">
        <div>需批核之修改價格 :
          <el-input-number v-model="creatFloatingAmountValue" :min="0" :max="100000000000000000" style="margin: 20px 0;"
            controls-position="right" @change="creatFindCard" />
        </div>
        <div>
